...
@Component({
  ...
  template: `
    <div class="shell">
      ...
      <!-- Below the sidebar-collapse breakpoint the sidebar is fully off-canvas
           (see app.scss) instead of an icon rail — this dims the content behind
           it while open and closes it on an outside tap, same pattern as the
           run drawer's own backdrop. -->
      @if (sidebarOpen()) {
        <button
          type="button"
      ...
      <!-- ─── Sidebar ─────────────────────────────────────────────────── -->
      <nav class="sidebar" [class.sidebar--open]="sidebarOpen()" aria-label="Main navigation">
        ...
        <ul class="sidebar__nav" role="list" aria-label="Pipelines">
          ...
          <li>
            <a routerLink="/showcase" routerLinkActive="sidebar__link--active"
               class="sidebar__link" aria-current="page" (click)="closeSidebar()">
        ...
        <ul class="sidebar__nav" role="list" aria-label="Compute">
          <li class="sidebar__group-label">Compute</li>
          <li>
            <a class="sidebar__link sidebar__link--disabled" aria-disabled="true"
               tabindex="-1">
        ...
        <ul class="sidebar__nav" role="list" aria-label="Data">
          <li class="sidebar__group-label">Data</li>
          <li>
            <a class="sidebar__link sidebar__link--disabled" aria-disabled="true"
               tabindex="-1">
      ...
      <div class="main-area">
        ...
        <header class="topbar" role="banner">
          <div class="topbar__left">
            <!-- Only ever visible below the sidebar-collapse breakpoint (see
                 app.scss) — above it the sidebar is always on-screen, so there's
                 nothing for this to toggle. -->
            <button
              type="button"
  ...
  styleUrl: './app.scss',
  host: {
    // Backstop for the same reason run-drawer listens at the document level:
    // reliable regardless of where focus currently is, not just while
    // something inside the sidebar happens to be focused.
...
export class App {
  ...
  /** Off-canvas sidebar open state — only meaningful below the sidebar-collapse breakpoint (app.scss). */
