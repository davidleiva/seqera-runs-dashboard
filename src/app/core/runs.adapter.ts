...
import { costFmt, durationFmt, formatSubmitted, shortProcessName } from './derive/format.utils';
import { attentionLabel, needsAttention } from './derive/attention.utils';
import { detectKnownIssue } from './derive/known-issues';
import { findFailedTask, taskBreakdown } from './derive/task.utils';
...
/** Narrows the `unknown` JSON boundary before anything downstream trusts the shape. */
export function isRawRun(value: unknown): value is RawRun {
  if (typeof value !== 'object' || value === null) return false;
  const r = value as Record<string, unknown>;
  return (
    typeof r['id'] === 'string' &&
    typeof r['runName'] === 'string' &&
    typeof r['projectName'] === 'string' &&
    typeof r['status'] === 'string' &&
    typeof r['submit'] === 'string' &&
    typeof r['load'] === 'object' &&
    r['load'] !== null
  );
}
...
function parseError(raw: RawRun): RunError | null {
  ...
  // Nextflow doesn't always format failures as "Caused by:" (e.g. a banner-style
  // message) — fall back to the raw message rather than claiming there's none.
  return { process, cause: cause ?? raw.errorMessage, raw: raw.errorMessage };
}
...
export function toRunDetailVM(raw: RawRun): RunDetailVM {
  ...
  const topProcesses = (raw.metrics ?? [])
    ...
    .map(m => ({
      name: shortProcessName(m.process),
  ...
  return {
    ...
    resources,
    topProcesses,
    knownIssue: detectKnownIssue(raw.errorMessage),
    failedTask: findFailedTask(raw),
  };
}
