...
> **Implementation note (post-audit).** The codebase briefly diverged into two
> parallel implementations of this screen (an original pass and an undocumented
> "v2" redesign built alongside it). The v2 redesign was promoted as canonical
> and the original was deleted — see `README.md`'s "A note on the two designs
> that used to live here" for the full rationale. One consequence: the
> **Health / glance layer** below describes individually-clickable KPI cards;
> what shipped is a **segmented status bar** (status = colour + icon + text,
> `aria-pressed` on the active segment, same calm/amber criticality headline)
> instead. It satisfies the same underlying requirements but does **not** yet
> have the removable filter **chip** called for below — tracked as a follow-up
> in the README's "What's deferred" table, not silently dropped. Read this
> section as the original intent; read the README for what's actually shipped.
...
- Structure:
  ...
  src/app/features/runs/{runs-page,runs-summary,runs-table,run-drawer}
  src/app/shared/      status-pill, task-bar, skeleton…
